...
- (void)commonInit
{
    ...
    _font = [UIFont systemFontOfSize:14*1];
    _height = 45;
    _selectionIndicator = [UIView new];
    _selectionIndicator.backgroundColor = [self customColor];
    ...
    [self setTitleColor:UIColorFromRGB(COLOR_CM_BLUE) forState:UIControlStateSelected];
    if (!IS_IOS6)
    {
        [self setTintColor:UIColorFromRGB(COLOR_CM_BLUE)];
    ...
    _showSeparators = YES;
    _separatorImage = [UIImage imageNamed:@"separator"];
}
...
- (void)layoutSubviews
{
    ...
    NSArray *separators = [self separators];
    CGFloat unit = [UIScreen mainScreen].bounds.size.width / self.items.count;
    __block CGFloat width = unit;
    ...
    [[self buttons] enumerateObjectsUsingBlock:^(UIButton *button, NSUInteger idx, BOOL *stop) {
        ...
        if (self.titleEdgeInset.top != 0
            || self.titleEdgeInset.left != 0
            || self.titleEdgeInset.bottom != 0
            || self.titleEdgeInset.right != 0) {
            [button setTitleEdgeInsets:self.titleEdgeInset];
        } else {
            [button setTitleEdgeInsets:UIEdgeInsetsMake(0.0f, 0.0f, topInset, 0.0f)];
        }
        
        UIImage *img = [UIImage imageNamed: self.iconItems[idx]];
        [button setImage:img forState:UIControlStateNormal];
        img = [UIImage imageNamed: self.iconHiItems[idx]];
        [button setImage:img forState:UIControlStateSelected];
        
        //separator
        if (self.showSeparators && separators && separators.count > idx) {
            rect.origin.x = width;
            rect.origin.y = rect.size.height/4;
            rect.size.width = 0.5;
            rect.size.height = rect.size.height/2;
            UIImageView *imgView = separators[idx];
            imgView.frame = rect;
            imgView.image = self.separatorImage;
            width += unit;
...
- (NSArray *)separators {
    NSMutableArray *separators = [NSMutableArray arrayWithCapacity: self.items.count-1];
...
- (UIColor *)titleColorForState:(UIControlState)state
{
    ...
    if (!color) {
        switch (state) {
            ...
            case UIControlStateHighlighted:         return [self customColor];
            case UIControlStateDisabled:            return [UIColor lightGrayColor];
            case UIControlStateSelected:            return [self customColor];
            default:                                return [self customColor];
        }
    }
...
-(UIColor *) customColor {
    if (IS_IOS6) {
        return [UIColor colorWithRed:0.053 green:0.438 blue:0.768 alpha:1.000];
    }
...
- (CGRect)selectionIndicatorRect
{
    ...
    if (self.autoAdjustSelectionIndicatorWidth) {
        ...
        CGSize size = MB_TEXTSIZE(title, _font);
        size.height = self.selectionIndicatorHeight;
        frame.size = size;
        ...
        [UIView animateWithDuration:duration
             ...
             usingSpringWithDamping:damping
              initialSpringVelocity:velocity
                         ...
                         animations:^{
                             self.selectionIndicator.frame = [self selectionIndicatorRect];
                             NSLog(@"SELECT frame : %f, %f", self.selectionIndicator.frame.size.width, self.selectionIndicator.frame.size.height);
                         }
                         completion:^(BOOL finished) {
                             [self enableAllButtonsInteraction:YES];
                             button.userInteractionEnabled = NO;
...
- (void)insertAllSegments
{
    ...
    for (int i = 0; i < self.numberOfSegments-1; i++) {
        [self addButtonSeparators:i];
    }
}

-(void)addButtonSeparators: (NSUInteger) segment {
    UIImageView *separator = [[UIImageView alloc] initWithFrame:CGRectZero];
    separator.tag = segment;
...
- (void)configureSegments
{
    ...
    self.selectionIndicator.frame = [self selectionIndicatorRect];
    self.selectionIndicator.backgroundColor = [self customColor];
}
